...
class Engine:
    ...
    def play_loop(self, game_id:str, min_val:int, max_val:int, player_a_id:str, player_b_id:str)->None:
        log.info("play_loop()")
        bind_contextvars(
            game_id=game_id,
            min_val=min_val,
        ...
        for i in range(1, max_attempts + 1):
            ...
            num = player_b.make_your_guess(game_id, min_val, max_val, max_attempts, i)
            typer.echo(f"Player A: please check if the guess is correct.")
            result: GuessFeedback = player_a.is_guess_number(game_id, min_val, max_val, max_attempts, i, num)
            match result:
                ...
                case GuessFeedback.EXACT:
                    ...
                    feedback: str | None = player_a.on_finished(game_id, min_val, max_val, max_attempts, attempt=i, is_win=False, reason=reason)
                    if feedback:
                        typer.secho(feedback, fg=typer.colors.GREEN, bold=True)
                    feedback = player_b.on_finished(game_id, min_val, max_val, max_attempts, attempt=i, is_win=True, reason=reason)
                    if feedback:
                        typer.secho(feedback, fg=typer.colors.GREEN, bold=True)
            ...
            if lowest + 1 >= highest:   #we must have at least 1 int number in interval [lowest, highest], where lowest<=highest
                ...
                feedback = player_a.on_finished(game_id, min_val, max_val, max_attempts, attempt=i, is_win=False, reason=reason)
                if feedback:
        ...
        feedback:str| None = player_a.on_finished(game_id, min_val, max_val, max_attempts, attempt=max_attempts, is_win=True, reason=reason)
        if feedback:
            typer.secho(feedback, fg=typer.colors.BRIGHT_BLACK, bold=False)
        feedback = player_b.on_finished(game_id, min_val, max_val, max_attempts, attempt=max_attempts, is_win=False, reason=reason)
        if feedback:
            typer.secho(feedback, fg=typer.colors.BRIGHT_BLACK, bold=False)
